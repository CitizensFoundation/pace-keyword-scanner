    ...
    def start_training(self):
        ...
        for options in trainingOptions:
            if (options.get("topic")):
                p = subprocess.Popen(["python", "model_prediction.py",f"{options.get('modelName')}",f"{options.get('topic')}"])
            else:
                p = subprocess.Popen(["python", "model_prediction.py",f"{options.get('modelName')}"])
            p.wait()
