...
public class WarcArchiveProcessor implements Runnable {

    private static final Logger logger = LogManager.getLogger(WarcArchiveProcessor.class);
    ...
    WarcArchiveProcessor(Semaphore schedulingSemaphore, Database patternDB, String archive,
    ...
    public void run() {
        try (final InputStream objectStream = new URL(archive).openStream();
            ...
            String currentDomain = null; // Should never be null in practice.

            String line;
            while ((line = contentReader.readLine()) != null) {
                if (line.startsWith(StringBundle.REQUEST_MARKER)) {
                    processingEntry = true;
                    currentDomain = null;
                } else if (processingEntry && line.startsWith(StringBundle.HTTP_HEADER_HOST)) {
                    currentDomain = getValue(line);
                } else if (processingEntry && currentDomain != null && line.startsWith(StringBundle.HTTP_HEADER_RESPONSE_OK)) {
                    ...
                    if (line == null) {
                        logger.warn("WARC entry incomplete due to archive split for domain {}. Skipping.", currentDomain);
                    ...
                    if (! contentType.toLowerCase().startsWith(StringBundle.ENTITY_TYPE)) {
                        logger.info("Unsupported entity {} found for {}. Skipping page.", contentType, currentDomain);
                        processingEntry = false;
                        continue;
                    }
                    ...
                    try {
                        boolean servesAds = doesPageServeAds(scanner, currentDomain, htmlContent.toString());

                        if (servesAds) {
                            logger.info("Domain {} serves ads.", currentDomain);
                            domainsWithAds.write(currentDomain);
                            domainsWithAds.write('\n');
                        } else {
                            logger.info("No ads served by {} domain.", currentDomain);
                            domainsWithoutAds.write(currentDomain);
